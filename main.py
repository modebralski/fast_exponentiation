...
def exponentiation(base, power):
    ...
    lenght_of_power_in_binary = len(power_in_binary) - 1
    squares_of_base = [base]
    for _ in range(lenght_of_power_in_binary):
    ...
    for square_of_base, current_bit in enumerate(power_in_binary[::-1]):
        if current_bit == "1":
            ...
            print(
                f"{base}^{2 ** square_of_base} * {current_bit} = {squares_of_base[square_of_base]}, result = {result}"
            )
        else:
